	...
	forwardVector(glm::vec3()), upVector(glm::vec3()), rightVector(glm::vec3()), worldUp(glm::vec3()), nearPlane(0.0f), farPlane(0.0f)
{
	...
	worldUp = upVector;
	nearPlane = 2.0f; //anything of front of 2 is not gonna be rendered
	farPlane = 50.0f; //anything beyond 50 is not gonna be rendered

	yaw = -90.0f;
	perspective = glm::perspective(fieldOfView, ClassEngine::GetInstance()->GetScreenSize().x /
		ClassEngine::GetInstance()->GetScreenSize().y,
		nearPlane, farPlane);

	orthographic = glm::ortho(0.0f, ClassEngine::GetInstance()->GetScreenSize().x,
		0.0f, ClassEngine::GetInstance()->GetScreenSize().y,
		-1.0f, 1.0f);
